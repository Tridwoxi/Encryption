...
public class Transformations {
    ...
    //john added this
    public static String substitution_shift(String input) {
        //takes string number input and base, and returns base 10 integer equivelent
    ...
    //rest of johns work starts here
//start of f function
    public static String f_function(String original, String k) {

        String xor_out = "";
        //XOR for input and k sub n

        for (int i = 0; i < original.length(); i++) {
            if (Integer.parseInt(String.valueOf(original.charAt(i)), 2)
        ...
        String s1 = xor_out.substring(0, 7);
        String s2 = xor_out.substring(8, 16);
        String s3 = xor_out.substring(17, 24);
        String s4 = xor_out.substring(25, 32);

        substitution_shift(s1);
        substitution_shift(s2);
        substitution_shift(s3);
        substitution_shift(s4);

        String s_shift_out = s1+s2+s3+s4;

        String[] perm_in = new String[32];
        for (int i = 0; i < perm_in.length; i++) {
            perm_in[i] = String.valueOf(s_shift_out.charAt(i));
        }

        String[] permuted = new String[32];
        permuted = permute(permuted);
        String f_function_out = "";

        for (int i = 0; i < permuted.length; i++)
            f_function_out += permuted[i];
